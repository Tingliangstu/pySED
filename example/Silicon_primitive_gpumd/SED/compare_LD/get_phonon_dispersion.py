...
def get_band_path(band_path, band_label, structure, use_seek_path=True, band_resolution=2001):
...
def phonon_calculator(relaxed_atoms, calculator, repeat_cell,
                      band_path, band_label, write_band_yaml=False,
                      force_constants=False, save_distance=True):
    """
    phonopy interface, get_force_constants function from calorine
    """
    
    phonon = get_force_constants(relaxed_atoms, calculator, [repeat_cell, repeat_cell, repeat_cell])
...
def get_calculator(structure, NEP_file, band_path, band_label, if_use_D3=False, repeat_cell=5):
    ...
    if if_use_D3:
        calc_D3 = DFTD3(method="PBE", damping="d3bj", params_tweaks={'s9': 0.0, 's8':0.7875, 'a1': 0.4289, 'a2': 4.4407})
        print("********* Plus D3 **********\n")
        calculator = SumCalculator([calculator_NEP, calc_D3])
    else:
        calculator = calculator_NEP

    structure.set_calculator(calculator)   ## Here, use the D3 calculator to optimize the monolayer structure (very important)
...
if __name__ == "__main__":
    ...
    band_label = [('Gamma', 'X'), ('X', 'U'), ('K', 'Gamma'), ('Gamma', 'L')]

    # Nep file
    nep_potential_file = 'nep.txt'
    get_calculator(structure, nep_potential_file, band_path, band_label)
