...
def plot_sed(out_files_name, plot_cutoff_freq = None, plot_interval = 5, vmin = None, vmax = None, if_show_figures=False):
    ...
    log = True
    color = 'Spectral'       # 'inferno', 'jet', 'Spectral', 'RdYlGn' (https://matplotlib.org/stable/tutorials/colors/colormaps.html)
    ...
    bar = fig.colorbar(im, ax=ax, extend='both', extendrect=True, label = r'log($\Phi $($\omega)$) (J.s)')
    bar.set_ticks(ticks)
    ...
    ax.set_ylabel(r'$\bfFrequency$ $\bf(THz)$', labelpad = 8.0, fontweight='normal', fontsize=fontsize)
    ax.tick_params(bottom=False, top=False, left=True, right=False)

    fig.suptitle(r'$\Phi $($\bfq$, $\bf\omega$)', x=0.48, y=0.95, fontweight='normal', fontsize=fontsize)

    if plot_cutoff_freq:
        ax.set_ylim([0, plot_cutoff_freq * scale_factor])

    plt.savefig('{}-SED.png'.format(out_files_name), format='png', dpi = 400, bbox_inches='tight')
...
if __name__ == "__main__":
    ...
    out_files_name = 'Silicon'
    plot_cutoff_freq = 20              # THz
    plot_interval = 5                  # THz
    vmax = -7                          # For SED image (high)
    vmin = -18                         # For SED image (low)

    plot_sed(out_files_name, plot_cutoff_freq, vmin=vmin, vmax=vmax)
