...
def plot_sed(out_files_name, plot_cutoff_freq = None, plot_interval = 5, vmin = None, vmax = None, if_show_figures=False):
    ...
    bar = fig.colorbar(im, ax=ax, label = r'log($\Phi $($\omega)$) (J.s)')
    bar.set_ticks(ticks)
...
if __name__ == "__main__":
    ...
    out_files_name = 'Alumnium_FCC'
    plot_cutoff_freq = 12              # THz
    plot_interval = 3                # THz
    vmax = -7                          # For SED image (high)
    vmin = -17                         # For SED image (low)

    plot_sed(out_files_name, plot_cutoff_freq, plot_interval=plot_interval)
